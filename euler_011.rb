...
def diagonals(grid, minsize)
  res = []
  revgrid = grid.map { |r| r.reverse }
  [grid, revgrid].each do |agrid|
    res << (0...grid.length).map { |i| agrid[i][i] }
    (1..grid.length-minsize).each do |x|
      dlength = grid.length - x
      res << (0...dlength).map { |y| agrid[x+y][y] }
      res << (0...dlength).map { |y| agrid[y][x+y] }
    end
  end
  res
end
...
minsize = 4
lines = rows(grid) + cols(grid) + diagonals(grid, minsize)
res = []
lines.each { |l| res += l.each_cons(minsize).to_a }
puts res.map { |arr| arr.reduce(:*) }.max
